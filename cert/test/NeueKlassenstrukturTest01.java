...
public class NeueKlassenstrukturTest01{
	...
	public void testeGeschaeftsobjekt(){
		...
		if(containsMethodName(Geschaeftsobjekt.class, "setNr"))
			fail("Die Nr eines Geschaeftsobjekts darf nach Objekterzeugung nicht mehr veraendert werden (read-only)!");
		if(containsMethodName(Geschaeftsobjekt.class, "setNummer"))
			fail("Die Nr eines Geschaeftsobjekts darf nach Objekterzeugung nicht mehr veraendert werden (read-only)!");
		if(!containsMethodName(Geschaeftsobjekt.class, "getNummer"))
	...
	public void testeKunde(){
		String[] required = {"getName", "getStrasse", "getOrt", "getNr", "setName", "setStrasse", "setOrt"};
		for(String method : required)
			if(!containsMethodName(Kunde.class, method))
	...
	public void testeProdukt(){
		...
		for(String method : required)
			if(!containsMethodName(Produkt.class, method))
		...
		assertEquals("Fehlerhafte Implementierung von getNr() der Klasse Produkt (nach Erzeugung eines neuen Artikels)", 12345, a.getNummer());
