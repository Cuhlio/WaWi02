...
import static test.TestRechnungsposition03.removeChar;
...
public class TestRechnung04{
	...
	public void testeRechnung(){
		...
		for(int i=nr+1; i<nr+100; i++){
			assertEquals("Nr einer erstellten Rechnung muss automatisch um 1 hochgezaehlt werden (Tipp: static-Attribut nutzen)!", i, new Rechnung().getNummer());
		}

		Rechnung r = new Rechnung();
		assertEquals("Der Status einer Rechnung muss bis zur Verbuchung IN_ERSTELLUNG sein!", Rechnungsstatus.IN_ERSTELLUNG, r.getRechnungsstatus());
		r.buchen();
...
//		}
//
//		assertTrue("Methode getRechnungspositionen liefert keine Liste ueber den Typ Rechnungsposition! Generics beachten!", genType.equals("class geschaeftsobjekte.Rechnungsposition"));
	...
	public void testeToString(){
		...
		assertEquals(
				"Fehler bei toString() einer Rechnung (Rechnung mit Kunde)",
				"Rechnung: "
						+ re.getNummer()
						+ "\nKunde: 4711\nName\nStr\nOrt\n\n"
						+ pos1tostring
		...
		assertEquals(
				"Fehler bei toString() einer Rechnung (Barverkauf, Rechnung ohne Kunde)",
				"Rechnung: "
						+ re.getNummer()
						+ "\nBarverkauf\n\n"
						+ pos1tostring
